...
%FILE PATH SETTING
FPNAME_ST.TXTCODE='ordch_fmri';
FPNAME_ST.EXFILE='OrderYourMindfMRI.m';
...
EXC_ST.NUMPTRIAL(1)=SEQ_ST.MAXSEQLENGTH*5;
...
EXC_ST.EX=str2double(EXC_ST.EXMODE)+1;
...
endB=(startB+numBinRun)-1;

%Set up intro
%RUNINTRO resets the trigger time!!
%ALWAYS GET A TRIGGER!
locShowMessege('RUNINTRO');


%Main Loop Structure=======================================================
for BLOCK=[startB:endB]
    ...
    while ~PREX.GO
        ...
        %Clean up RECORD_ST?? (this is not recorded in fMRI session)
        RECORD_ST.EV_BLEND(BLOCK,:)=GetSecs-TRIGERTS;
        
        %Block result (NEED THIS???)
        PREX.GO=true;
        %if BLOCK < endB && EXC_ST.EX==1,locShowMessege('BRESULT');end;

        %End Screen
        if BLOCK == endB,locShowMessege('EXEND');break;end;
    end
...
switch workT
    case {'PRETRIAL'},color=COLOR_ST.RED;fsize=STIM_ST.FONTSIZE+50;
    case{'NEUTRAL'},color=COLOR_ST.BLACK;fsize=STIM_ST.FONTSIZE+50;
end
...
im=CenterRectOnPoint(STIM_ST.STIMRECT,STIM_ST.XC,STIM_ST.YC+STIM_ST.FCADJ-10);
...
%Consistant settings
Screen('FillRect',W,COLOR_ST.BACKG);
Screen('TextSize',W,STIM_ST.FONTSIZE+15);
...
switch msgtime
    ...
    case {'RUNINTRO'}% Begining of run
        ...
        TEXT(3)={''};
        TEXT(4)={''};
        TEXT(5)={'Please wait a moment, the run will begin shortly.'};
        C={'BLACK','BLACK','BLACK','BLACK','BLACK'};
                
    case {'BRESULT'}% Ending Block BREAK screen (no money)
        [ConText]=locCondMessage(msgtime);
        TEXT(1)={['You have completed block ',num2str(BLOCK),' of ',num2str(EXC_ST.NUMTBLOCK(EXC_ST.EX))]};
        TEXT(2)={ConText{1}};
        TEXT(3)={ConText{2}};
        TEXT(4)={''};
        TEXT(5)={'Press any key to proceed'};
        C=repmat({'BLACK'},1,size(TEXT,2));
        
    case {'EXEND'}% Ending Experiment screen
        TEXT(1)={'Thank you'};
        TEXT(2)={'You have completed one run!'};
        TEXT(3)={''};
        TEXT(4)={''};
        TEXT(5)={'Press any key to proceed'};
        C=repmat({'BLACK'},1,size(TEXT,2));
...
%Drawing
for i=1:size(TEXT,2)
    if strcmp(msgtime,'BRULE'),adj=50;else adj=80;end;
